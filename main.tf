...
resource "cloudflare_workers_route" "enabled" {
  for_each = local.should_create_routes && length(var.enabled_routes) > 0 ? toset(var.enabled_routes) : toset([])
  zone_id  = var.zone_id
...
resource "cloudflare_workers_route" "disabled" {
  for_each = local.should_create_routes && length(var.disabled_routes) > 0 ? toset(var.disabled_routes) : toset([])
  zone_id  = var.zone_id
  pattern  = format("%s%s", var.domain_name, each.value)
  script   = null
}
